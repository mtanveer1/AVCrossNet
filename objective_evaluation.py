...
import torch
# from asteroid.metrics import si_sdr
...
def run_sisdr(target, enhanced):
    return sisdr(torch.tensor(enhanced), torch.tensor(target))
...
def run_metrics(scene, enhanced, target, cfg):
    ...
    m_sisdr = float(run_sisdr(clean, enh))
    ...
    with open(scene_metrics_file, "w") as csv_f:
        ...
        csv_writer.writerow([scene_name, m_stoi, m_pesq, m_sisdr])
